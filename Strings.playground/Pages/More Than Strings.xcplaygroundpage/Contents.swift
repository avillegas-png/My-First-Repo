...
let loses = 98
let draws = 7
let undefeatedStreak = 19
let rank = "Grand Champion"
let playerStats = "\(playerName) has joined the server. \(playerName) has \(wins) wins, \(loses) loses, and \(draws) draws. They are currently on a \(undefeatedStreak) undefeated streak. They are currently ranked at \(rank)!"
//:
//:
